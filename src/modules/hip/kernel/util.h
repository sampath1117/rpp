...
__host__ __device__ constexpr T *begin(T (&array)[N]) noexcept { return array; }
template <typename T, size_t N>
__host__ __device__ constexpr T *end(T (&array)[N]) noexcept { return array + N; }


template <typename T, typename = std::enable_if_t<!std::is_const<T>::value>>
__host__ __device__ constexpr auto begin(T &collection)->decltype(collection.begin()) {
  return collection.begin();
}


template <typename T, typename = std::enable_if_t<!std::is_const<T>::value>>
__host__ __device__ constexpr auto end(T &collection)->decltype(collection.end()) {
  return collection.end();
}


template <typename T>
__host__ __device__ constexpr auto begin(const T &collection)->decltype(collection.begin()) {
  return collection.begin();
}


template <typename T>
__host__ __device__ constexpr auto end(const T &collection)->decltype(collection.end()) {
  return collection.end();
}
...
__host__ __device__ auto size(const Collection &c)->decltype(c.size()) {
  return c.size();
}
