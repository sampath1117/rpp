...
// -------------------- Set 2 - cast kernels --------------------
template <typename T, typename U>
__global__ void cast_1d_hip_tensor(T *srcPtr,
                                  uint srcStrides,
                                  U *dstPtr,
                                  uint dstStrides,
                                  uint *roiTensor)
...
template <typename T, typename U>
__global__ void cast_2d_hip_tensor(T *srcPtr,
                                  uint2 srcStridesNH,
                                  U *dstPtr,
                                  uint2 dstStridesNH,
                                  uint *roiTensor)
...
template <typename T, typename U>
__global__ void cast_3d_hip_tensor(T *srcPtr,
                                  uint2 srcStridesDH,
                                  U *dstPtr,
                                  uint2 dstStridesDH,
                                  uint *roiTensor)
...
template <typename T, typename U>
__global__ void cast_nd_hip_tensor(T *srcPtr,
                                  uint *srcStrides,
                                  uint *srcDims,
                                  uint numDims,
                                  U *dstPtr,
...
template <typename T, typename U>
RppStatus hip_exec_cast_generic_tensor(T *srcPtr,
                                      ...
                                      rpp::Handle& handle)
{
    ...
    if (numDims == 1)
    {
        ...
        hipLaunchKernelGGL(cast_1d_hip_tensor,
    ...
    else if (numDims == 2)
    {
        ...
        hipLaunchKernelGGL(cast_2d_hip_tensor,
                           dim3(ceil((float)globalThreads_x/LOCAL_THREADS_X), ceil((float)globalThreads_y/LOCAL_THREADS_Y), ceil((float)globalThreads_z/LOCAL_THREADS_Z)),
                           dim3(LOCAL_THREADS_X, LOCAL_THREADS_Y, LOCAL_THREADS_Z),
                           0,
                           handle.GetStream(),
    ...
    else if (numDims == 3)
    {
        ...
        {
            hipLaunchKernelGGL(cast_3d_hip_tensor,
                               dim3(ceil((float)globalThreads_x/LOCAL_THREADS_X), ceil((float)globalThreads_y/LOCAL_THREADS_Y), ceil((float)globalThreads_z/LOCAL_THREADS_Z)),
                               dim3(LOCAL_THREADS_X, LOCAL_THREADS_Y, LOCAL_THREADS_Z),
                               0,
                               handle.GetStream(),
    ...
    else
    {
        ...
        hipLaunchKernelGGL(cast_nd_hip_tensor,
