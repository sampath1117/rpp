...
int main(int argc, char **argv)
{
    ...
    hipHostMalloc(&srcImgSizes, batchSize * sizeof(RpptImagePatch));
   ...
   // Read images
    ...
    for (int perfRunCount = 0; perfRunCount < numIterations; perfRunCount++)
    {
        for(int t = 0; t < (int)imageNames.size() / batchSize; t++)
        {
            ...
            if(decoderType == 0)
            {
                read_image_batch_turbojpeg(inputu8, srcDescPtr, imageNamesPath, roiTensorPtrSrc, roiTensorPtrDst);
                read_image_batch_turbojpeg(inputu8Second, srcDescPtr, imageNamesPathSecond, roiTensorPtrSrc, roiTensorPtrDst);
            }
            else
            {
                read_image_batch_opencv(inputu8, srcDescPtr, imageNamesPath, roiTensorPtrSrc, roiTensorPtrDst);
                read_image_batch_opencv(inputu8Second, srcDescPtr, imageNamesPathSecond, roiTensorPtrSrc, roiTensorPtrDst);
            }
            if (layoutType == 1)
            {
                // Convert default OpenCV PKD3 to PLN3 for first and second input batch
